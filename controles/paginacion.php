...
if ($num_total_rows > 0) {
    ...
    $con->conect();
        ...
        'SELECT * FROM equipos ORDER BY id DESC LIMIT '.$start.', '.$pag);
                ...
                $td="";
               $td="
               <table class='table' id='table-checkbox'>
               <tr>
                    <th>Imagen</th>
      ...
      while($resul = $result->fetch_assoc()){
                ...
                $tabla="equipos";
                    $td .="<tr>";
                    $td .= "<td><img src='img/".$marca."/".$imagen."' width='100px'></td>";
                    $td .= "<td>".$pdf."</td>";
                    $td .= "<td>".$titulo."</td>";
                    $td .= "<td>".$marca."</td>";
                    $td .= '<td><button class="btn btn-warning" onclick="sweetdel('.$id.')">Eliminar</button></td>';
                    $td .= '<td><!-- Default checked -->
<div class="custom-control custom-checkbox">
  <input type="checkbox" class="custom-control-input" id="defaultChecked'.$id.'" name="del[]" value="'.$id.'">
  <label class="custom-control-label" for="defaultChecked'.$id.'"></label>
</div></td>';
                    $td .="</tr>";
                }
                $td.="</table>";
